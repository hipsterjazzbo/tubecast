...
| `env_override` | Manage the personal, gitignored `.env.lerd_override` (`KEY=VALUE` pairs win over lerd's defaults on `env_setup`; `LERD_EXTERNAL_SERVICES=<svc,svc>` marks services lerd writes vars for but won't start) |
...
| `dumps_recent` / `dumps_status` / `dumps_clear` / `dumps_toggle` | Inspect / clear / toggle the lerd debug bridge that captures `dump()` / `dd()` calls. Off by default; enable with `dumps_toggle({enable: true})` |
| `analyze_queries` | N+1 and slow-query report over captured queries, grouped per request with the `file:line` to fix (loop: `dumps_toggle` enable → `dumps_clear` → hit the page → `analyze_queries`) |
...
| `service_config` | Read/write/restore/reset/list_backups a service's runtime tuning override (built-in mysql/mariadb/redis; custom services opt in via `tuning:` in YAML) |
| `service_check_updates` | Check the registry for newer service images (safe in-strategy update vs cross-strategy upgrade); omit `name` to scan all active |
...
| `workers_health` | List failed worker units grouped per site (read-only) |
| `workers_heal` | Reset and restart failed worker units; pass `unit` for one, omit to heal all (never edits config) |
| `commands_list` | List on-demand framework commands for a site (framework defaults merged with the project's `.lerd.yaml` `commands:` block) |
| `commands_run` | Run a framework command on a site by name; `force: true` is required for confirm-gated commands |
| `command_add` | Add or update a project command in `.lerd.yaml`'s `commands:` block (same name as a framework default replaces it) |
| `command_remove` | Remove a project command from `.lerd.yaml` by name (use `command_add` with `disabled: true` to hide a framework default) |
...
| `framework_search` | Search the community framework store by name or label |
| `framework_install` | Install a framework from the community store (auto-detects version from `composer.lock` if omitted) |
...
| `site_nginx` | Read/write/reset a site's custom nginx override (saving runs `nginx -t`, backs up the prior file, reloads); `branch` targets a worktree |
...
| `dns_diagnose` | Walk the DNS chain (container, config, port 5300, dig, resolver, interface routing, system lookup) and report which rung is broken |
