...
use App\Requests\StoreSourceRequest;
use App\Requests\UpdateSourceSettingsRequest;
use App\Services\Podcast\PodcastFeedService;
use App\Services\Download\DownloadProgressService;
use App\Services\Source\EpisodeFilterService;
use App\Services\Indexing\IndexingProgressService;
use App\Services\Indexing\IndexingProgress;
use App\Services\Source\SourceDeletionService;
...
final readonly class SourceController
{
    ...
    #[Post('/sources')]
    public function store(StoreSourceRequest $request): Redirect
    {
        $url = $request->url;
        ...
        $filters = $request->toSourceFilters();
        ...
        $source = Source::create(
            ...
            mediaProfileId: $request->parsedMediaProfileId(),
            includeShorts: $request->includeShorts,
            includeLive: $request->includeLive,
            saveVideo: $request->saveVideo,
            saveAudio: $request->saveAudio,
    ...
    public function updateSettings(Source $source, UpdateSourceSettingsRequest $request): Redirect
    {
        $sourceId = ModelId::int($source->id);
        ...
        $source->includeShorts = $request->includeShorts;
        $source->includeLive = $request->includeLive;
        $source->saveVideo = $request->saveVideo;
        $source->saveAudio = $request->saveAudio;
